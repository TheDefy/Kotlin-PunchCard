 ...
 * @author panghui
 */
public class NetDiagnoService extends
        NetAsyncTaskEx<String, String, String> implements NetPing.LDNetPingListener,
        NetTraceRoute.LDNetTraceRouteListener, NetSocket.LDNetSocketListener {
    private String _appCode; // 客户端标记
    private String _appName;
    private String _appVersion;
    private String _UID; // 用户ID
    private String _deviceID; // 客户端机器ID，如果不传入会默认取API提供的机器ID
    private String _domain; // 接口域名
    private String _pingDomain;
    ...
    private NetTraceRoute _traceRouter; // 监控ping模拟traceroute的执行过程
     ...
     * @param theDomain
     */
    public NetDiagnoService(Context context, String theAppCode,
                            String theAppName, String theAppVersion, String theUID,
                            String theDeviceID, String theDomain, String pingDomain,
                            String theCarrierName,
                            NetDiagnoListener theListener) {
        super();
    ...
    public String startNetDiagnosis() {
        if (TextUtils.isEmpty(this._domain))
            return "";
        ...
        if (_isNetConnected) {
            ...
            _isSocketConnected = _netSocker.exec(_domain);

            // 诊断ping信息, 同步过程
            recordStepInfo("\n开始ping...");
            _netPinger = new NetPing(this, 4);
            recordStepInfo("ping " + _pingDomain);
            _netPinger.exec(_pingDomain, false);
            if (!(_isNetConnected && _isDomainParseOk && _isSocketConnected)) {// 联网&&DNS解析成功&&connect测试成功
                ...
                if (NetUtil.NETWORKTYPE_WIFI.equals(_netType)) {// 在wifi下ping网关
            ...
            // 开始诊断traceRoute
            recordStepInfo("\n开始traceroute...");
            _traceRouter = NetTraceRoute.getInstance();
            _traceRouter.initListenter(this);
            _traceRouter.isCTrace = this._isUseJNICTrace;
            _traceRouter.startTraceRoute(_domain);
    ...
    private void recordCurrentAppVersion() {
        ...
        if (TextUtils.isEmpty(_carrierName)) {
            _carrierName = NetUtil.getMobileOperator(_context);
        }
        recordStepInfo("学校:\t" + _carrierName);
    ...
    private void recordLocalNetEnvironmentInfo() {
        ...
        // 网络状态
        if (NetUtil.isNetworkConnected(_context)) {
        ...
        // 获取当前网络类型
        _netType = NetUtil.getNetWorkType(_context);
        recordStepInfo("当前联网类型:\t" + _netType);
        if (_isNetConnected) {
            if (NetUtil.NETWORKTYPE_WIFI.equals(_netType)) { // wifi：获取本地ip和网关，其他类型：只获取ip
                _localIp = NetUtil.getLocalIpByWifi(_context);
                _gateWay = NetUtil.pingGateWayInWifi(_context);
            } else {
                _localIp = NetUtil.getLocalIpBy3G();
            }
        ...
        // 获取本地DNS地址
        if (_isNetConnected) {
            _dns1 = NetUtil.getLocalDns("dns1");
            _dns2 = NetUtil.getLocalDns("dns2");
        ...
        // 获取远端域名的DNS解析地址
        if (_isNetConnected) {
            recordStepInfo("远端域名:\t" + this._domain);
            _isDomainParseOk = parseDomain(this._domain);// 域名解析
        }
    }
    ...
    private boolean parseDomain(String _dormain) {
        ...
        int len = 0;
        String ipString = "";
        Map<String, Object> map = NetUtil.getDomainIp(_dormain);
        ...
        if (_remoteInet != null) {// 解析正确
            ...
            if (Integer.parseInt(useTime) > 10000) {
                map = NetUtil.getDomainIp(_dormain);
    ...
    private String requestOperatorInfo() {
        String res = null;
        String url = NetUtil.OPERATOR_URL;
        ...
        try {
            ...
            if (responseCode == 200) {
                res = NetUtil.getStringFromStream(conn.getInputStream());
                if (conn != null) {
                    conn.disconnect();
                }
            }

...
import com.bbtree.baselib.net.HttpClient;
...
public class RxUtils {
     ...
     * @return
     */
    public static Observable<String> postMapNoCrypt(final String path, final Map o) {
        return Observable.just(path)
                .subscribeOn(Schedulers.io())
                .map(new Function<String, String>() {
                    @Override
                    public String apply(String s) {
                        try {
                            return client.postMapNoCrypt(path, o);
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                        return "";
